    ...
    <MainLayout portalName="פורטל מרצים" view="lecturer">
      <div className="space-y-8 animate-in fade-in duration-500 max-w-6xl mx-auto pb-12">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end border-b border-gray-200 pb-6 gap-4">
          <div>
            <h1 className="text-3xl font-extrabold text-[#00857e]">ניהול קורסים</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {courses.map(course => (
            <Link key={course.id} to={`/lecturer/courses/${course.id}`} className={`group bg-white rounded-3xl shadow-sm border border-gray-100 p-6 hover:shadow-xl ${course.accent.borderHover} transition-all duration-300 flex flex-col h-full hover:-translate-y-1`}>
